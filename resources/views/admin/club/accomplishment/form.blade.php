...
@if($mode == 'edit')
@section('title', 'Edit Prestasi UKM | UKM Admin')
@else
@section('title', 'Buat Prestasi UKM | UKM Admin')
@endif
...
<main class="h-screen w-screen flex">
  ...
  <div class="min-h-screen bg-neutral-100 w-full">
    <h1 class="font-medium h-16 flex items-center text-xl px-6 py-4 bg-white">{{ $mode == 'edit' ? 'Edit' : 'Buat' }} Prestasi UKM</h1>
    <div class="flex w-full mt-8 flex-row gap-x-12 justify-center">
      <div class="flex flex-col items-center py-6 w-full max-w-96">
        <form action="{{ $mode == 'edit' ? route('admin.club.accomplishment.edit', ['clubId' => $club->id, 'id' => $data->id]) : route('admin.club.accomplishment.create', ['clubId' => $club->id]) }}" enctype="multipart/form-data" method="post" class="flex flex-col items-center w-full">
          @csrf
          @if($mode == 'edit')
          @method('PUT')
          @endif
          <div class="w-full">
            ...
            <textarea rows="3" id="title-input" placeholder="Input judul prestasi" name="title" class="resize-none bg-neutral-50 border border-neutral-300 text-neutral-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">{{ old('title', $data->title ?? '') }}</textarea>
            @error('title')
