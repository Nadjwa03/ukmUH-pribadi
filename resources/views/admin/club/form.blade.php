    ...
    <div class="flex flex-col items-center mt-8 px-6 py-6">
      <form action="{{ $mode == 'edit' ? route('admin.club.edit', $data->id) : route('admin.club.create') }}" enctype="multipart/form-data" method="post" class="flex flex-col items-center max-w-96 w-full">
        ...
        <div class="w-full mt-4">
          ...
          <textarea id="about-input" name="about" rows="4" class="block p-2.5 w-full text-sm text-neutral-900 bg-neutral-50 rounded-lg border border-neutral-300 focus:ring-blue-500 focus:border-blue-500" placeholder="Tuliskan deskripsi singkat tentang UKM ini...">{{ old('about', $data->about ?? '') }}</textarea>
        </div>
        <div class="w-full mt-4">
          <label for="history-input" class="block mb-2 text-sm font-medium text-neutral-900">Sejarah UKM</label>
          <textarea id="history-input" name="history" rows="4" class="block p-2.5 w-full text-sm text-neutral-900 bg-neutral-50 rounded-lg border border-neutral-300 focus:ring-blue-500 focus:border-blue-500" placeholder="Tuliskan deskripsi singkat tentang UKM ini...">{{ old('about', $data->about ?? '') }}</textarea>
        </div>
